<x-layouts.app title="Buat Penugasan">
    @push('styles')
        @vite(['resources/css/create-wi.css'])
    @endpush

    <div class="container-fluid p-3 p-lg-4">
        @php
            $showRunningText = true; // Set ke false untuk menonaktifkan
            $pesanRunningText = "PENGUMUMAN: Kapasitas Workcenter diaktifkan !!, Kapasitas Workcenter akan di update pada pukul 15.30 setiap hari nya.";
        @endphp

        @if($showRunningText)
        <div class="running-text-wrapper mb-3 shadow-sm border">
            <div class="running-text-content py-2">
                <div class="marquee">
                    <span><i class="fa-solid fa-circle-info me-2"></i> {{ $pesanRunningText }} &nbsp;&nbsp; <i class="fa-solid fa-bullhorn mx-2"></i> {{ $pesanRunningText }}</span>
                </div>
            </div>
        </div>
        @endif
        ...
        <script>
            ...
            function processDrop(evt, item, toList, fromList, targetWcId) {
                ...
                if (hasChildren) {
                    ...
                    children.forEach(c => {
                        let k = parseFloat(String(c.kapaz).replace(',', '.')) || 0;
                        if (k === 0) k = 9.5;
                        totalChildMins += (k * 60);
                    });
                            ...
                            <div class="card-body p-2"> 
                                ...
                                <div class="row g-2 collapse" id="topCapacityContainer">
                                    ...
                                    ${children.map(child => {
                                        ...
                                        const consumedSec = CONSUMED_MAP[child.code] || 0;
                                        const consumedMin = consumedSec / 60;
                                        const pct = limit > 0 ? (consumedMin / limit) * 100 : 0;
                                        
                                        const barClass = consumedMin > limit ? 'bg-danger' : 'bg-secondary';
                                        const textClass = consumedMin > limit ? 'text-danger' : '';

                                        return `
                                        <div class="col-md-6">
                                            <div class="d-flex justify-content-between text-xs fw-bold mb-1">
                                                <span>${child.code} - ${child.name}</span>
                                                <span id="cap-text-${child.code}" class="${textClass}">${consumedMin.toFixed(1)} / ${limit.toFixed(1)} Min</span>
                                            </div>
                                            <div class="progress" style="height: 6px;">
                                                <div id="cap-bar-${child.code}" class="progress-bar ${barClass}" role="progressbar" style="width: ${Math.min(pct, 100)}%"></div>
                                            </div>
                                        </div>
                                    `;}).join('')}
                                </div>
            ...
            window.updateCardSummary = function(TriggerAufnr) {
                ...
                if (targetWcId) {
                    ...
                    if (PARENT_WORKCENTERS.hasOwnProperty(normalizedTargetWc)) {
                        ...
                        // [NEW] Initialize with DB Consumption
                        children.forEach(child => {
                            const childCode = child.code;
                            const dbSec = CONSUMED_MAP[childCode] || 0;
                            const dbMin = dbSec / 60;
                            
                            // If not present in globalChildUsage (meaning no draft cards for this child yet),
                            // ensure we at least start with DB usage.
                            // BUT globalChildUsage actually accumulates *draft* usage.
                            // So total = dbMin + (globalChildUsage[childCode] || 0)
                            
                            // Since globalChildUsage is rebuilt from scratch every time updateCardSummary runs (lines 1321-1332 + assignments),
                            // we can separate them or just sum them up here for display.
                            // Let's keep globalChildUsage as "Current Draft Usage" for clarity, and add DB usage at display time.
                        });

                        // Make available for validations
                        window.currentGlobalDraftUsage = globalChildUsage; 

                        children.forEach(child => {
                            const childCode = child.code;
                            let rawKapaz = parseFloat(String(child.kapaz).replace(',', '.')) || 0;
                            if (rawKapaz === 0) rawKapaz = 9.5; 
                            const thisChildLimit = rawKapaz * 60; 
                            
                            const draftUsage = globalChildUsage[childCode] || 0;
                            const dbSec = CONSUMED_MAP[childCode] || 0;
                            const totalUsage = (dbSec / 60) + draftUsage;

                            const pct = thisChildLimit > 0 ? (totalUsage / thisChildLimit) * 100 : 0;
                            ...
                            if (bar && text) {
                                ...
                                text.innerText = `${totalUsage.toFixed(1)} / ${thisChildLimit.toFixed(1)} Min`;
                                
                                if (totalUsage > thisChildLimit) {
            ...
            window.updateChildWCOptions = function(aufnr) {
                ...
                selects.forEach(sel => {
                    ...
                    // Need limits for checking
                    let limits = {};
                    let normalizedTargetWc = (card.dataset.targetWc || '').toUpperCase();
                    if (PARENT_WORKCENTERS[normalizedTargetWc]) {
                        PARENT_WORKCENTERS[normalizedTargetWc].forEach(c => {
                            let k = parseFloat(String(c.kapaz).replace(',', '.')) || 0;
                            if (k === 0) k = 9.5;
                            limits[c.code] = k * 60;
                        });
                    ...
                    options.forEach(opt => {
                        ...
                        const totalUsage = (dbSec / 60) + draftUsage;
                        const limit = limits[wcCode] || 570; // Fallback
                        
                        const isFull = (totalUsage >= limit);
